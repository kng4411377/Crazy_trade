...
### 3. **UUID Database Error** ✅
**Problem:**
```json
{"error": "Error binding parameter 3: type 'UUID' is not supported"}
```

**Root Cause:**  
Alpaca returns order IDs as UUID objects (e.g., `UUID('c8d1892d-...')`), but SQLite doesn't support UUID types. Database columns were defined as `Integer`, causing type errors.

**Solution:**  
Modified database schema and order ID handling:
- Changed all order ID columns from `Integer` to `String`
- Convert UUID objects to strings before storing: `str(order.id)`
- Backed up and recreated database with new schema

**Code Changes:**
- Updated `src/database.py` - SymbolState, OrderRecord, FillRecord schemas
- Updated `src/state_machine.py` - lines 129, 137, 330, 337, 354
- Updated `src/bot.py` - lines 306, 359
- Database migrated: `bot.db` → `bot.db.backup_YYYYMMDD_HHMMSS`

**Details:** See `docs/UUID_FIX_SUMMARY.md`

---

### 4. **Crypto Price Fetching Error** ✅
**Problem:**
```json
...
- `src/database.py` - Changed order ID columns from Integer to String (UUID support)
- `src/state_machine.py` - Convert UUIDs to strings before storing
- `src/bot.py` - Convert UUIDs to strings before storing
...
- `docs/UUID_FIX_SUMMARY.md` - New (UUID database fix details)
